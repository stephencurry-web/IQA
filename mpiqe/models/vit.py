...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.scale = qk_scale or head_dim**-0.5     #根号dk分之一
    ...
    def forward(self, x):
        ...
        # reshape: -> [batch_size, num_patches + 1, 3(拆封为qkv三个), num_heads, embed_dim_per_head]
        # permute（调整数据顺序）: -> [3, batch_size, num_heads, num_patches + 1, embed_dim_per_head]
        qkv = (
            self.qkv(x)
        ...
        # transpose: -> [batch_size, num_heads, embed_dim_per_head, num_patches + 1]   矩阵乘法调换顺序
        # @: multiply -> [batch_size, num_heads, num_patches + 1, num_patches + 1]  矩阵乘法为多维的时候  对最后两个维度乘
        attn = q @ k.transpose(-2, -1)
        attn = attn.softmax(dim=-1)  #对每一行softmax
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        qk_scale=None,
        drop=0.0,        #MHA使用的
        attn_drop=0.0,    #softmax(qk/根号dk)全连接层后的
        drop_path=0.0,
        act_layer=nn.GELU,
...
class Experts_MOS(nn.Module):
    ...
    def forward(self, x, ref):
        ...
        ref = ref.view(B, 1, -1)    #调整形状，调成一行
        ref = ref.expand(B, self.juery, -1)
        output_embedding = bunch_embedding + ref
        x = self.bunch_decoder(output_embedding, x)   #x是encoder的输出
        x = self.heads(x)
        x = x.view(B, -1).mean(dim=1)   #对第二维求平均 abc->ac
        return x.view(B, 1)
...
class vit_models(nn.Module):
    ...
    def __init__(
        ...
        num_patches = 196
        self.cls_token = nn.Parameter(torch.randn(1, 1, embed_dim))  #(第一个1是batch维度)
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))    #cls不用加postion？
        ...
        trunc_normal_(self.pos_embed, std=0.02)       #截断正态分布
...
def build_vit(
    ...
    mlp_ratio=4,
    qkv_bias=True,
    norm_layer=partial(nn.LayerNorm, eps=1e-6),    #partial用来固定参数
...
if __name__ == "__main__":
    vit = build_vit(
        pretrained=True,
        pretrained_model_path="/home/gtm/IQA/Data-Eff-IQA/deit_3_small_224_1k.pth",
    )
